...
Edge Env::step(TreeNode *node, Action *action)
{
    ...
    vector<bool> action_mask = this->action_space->get_action_mask(vocab_i);
    TreeNode *new_node = new TreeNode(vocab_i, node->end_node, action->action_id, node, action_mask);
